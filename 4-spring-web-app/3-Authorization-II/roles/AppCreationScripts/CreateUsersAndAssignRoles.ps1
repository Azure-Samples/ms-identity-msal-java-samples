...
#> 
    ...
    if ($app)
    {
        ...
        $privilegedAdminRole = $servicePrincipal.AppRoles | Where-Object { $_.DisplayName -eq "PrivilegedAdmin" }
        $regularUserRole = $servicePrincipal.AppRoles | Where-Object { $_.DisplayName -eq "RegularUser" }
        ...
        # Create users
        # ------
        # Make sure that the user who is running this script is assigned to the Privileged Admin role
        Write-Host "Adding '$($user.DisplayName)' as a member of the '$($privilegedAdminRole.DisplayName)' role"
        $userAssignment = New-AzureADUserAppRoleAssignment -ObjectId $user.ObjectId -PrincipalId $user.ObjectId -ResourceId $servicePrincipal.ObjectId -Id $privilegedAdminRole.Id

        # Creating a Privileged Admin
        Write-Host "Creating a new user and assigning to '$($privilegedAdminRole.DisplayName)' role"
        $aPrivilegedAdmin = CreateUserRepresentingAppRole $appName $privilegedAdminRole $tenantName
        $userAssignment = New-AzureADUserAppRoleAssignment -ObjectId $aPrivilegedAdmin.ObjectId -PrincipalId $aPrivilegedAdmin.ObjectId -ResourceId $servicePrincipal.ObjectId -Id $privilegedAdminRole.Id
        Write-Host "Created user "($aPrivilegedAdmin.UserPrincipalName)" with password 'test123456789.'"

        # Creating a Regular User
        Write-Host "Creating a user and assigning to '$($regularUserRole.DisplayName)' role"
        $aRegularUser = CreateUserRepresentingAppRole $appName $regularUserRole $tenantName
        $userAssignment = New-AzureADUserAppRoleAssignment -ObjectId $aRegularUser.ObjectId -PrincipalId $aRegularUser.ObjectId -ResourceId $servicePrincipal.ObjectId -Id $regularUserRole.Id
