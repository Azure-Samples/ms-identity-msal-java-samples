...
2. Find the key `Enter_Your_Client_ID_Here` and replace the existing value with the application ID (clientId) of `java-spring-webapp-groups` app copied from the Microsoft Entra admin center.
3. Find the key `Enter_Your_Client_Secret_Here` and replace the existing value with the key you saved during the creation of `java-spring-webapp-groups` copied from the Microsoft Entra admin center.
...
#### Update the Microsoft Entra app registration (java-spring-webapp-groups)
