...
1. Update your **Microsoft Entra app Registration**'s redirect URIs from the **Microsoft Entra admin center**, in order to include the redirect URI of your deployed Java Servlet application.
...
2. After you've deployed your app in the next sections, come back and add the secrets from a secure location such as:
   ...
   2. **Environment Variables** (*Microsoft Entra admin center > App Services > `<Your App Name>` > Configuration*). Go to the Microsoft Entra admin center and set the value for `CLIENT_SECRET`. Now, back in your local code, modify the following lines in the `helpers/Config.java` file as follows:
